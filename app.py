...
<style>
    ...
    .big-val { font-size: 28px; font-weight: bold; }
    .label-card { font-size: 13px; font-weight: 600; text-transform: uppercase; }
    ...
    .grid-label { color: #8E8E93; font-size: 11px; font-weight: bold; }
    .grid-value { color: #FFFFFF; font-size: 19px; font-weight: bold; }
...
km_b = st.session_state.bruto / st.session_state.km
km_l = liq / st.session_state.km
hr_b = st.session_state.bruto / st.session_state.horas
hr_l = liq / st.session_state.horas
viagens = max(1, round(st.session_state.bruto / 30))
...
with tab_lan:
    ...
    st.session_state.horas = st.number_input("Horas de Trabalho", value=st.session_state.horas, min_value=0.1)
...
# --- ABA 3: HISTÓRICO ---
with tab_hist:
    st.subheader("Ganhos Semanais/Mensais")
